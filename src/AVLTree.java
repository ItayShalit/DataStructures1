...
public class AVLTree {
  ...
  public AVLTree(IAVLNode root_node) {
	  ...
	  this.getRoot().setRight(new AVLNode(-1, null, -1, this.getRoot(), 0));
	  this.getRoot().setLeft(new AVLNode(-1, null, -1, this.getRoot(), 0));
   ...
   */
   ...
   public int insert_node(IAVLNode node) {
	  ...
	  if (this.is_key_in_tree(node.getKey())) { //If the key is already in the tree, we return -1 and do not implement changes to the tree.
		  return -1;
	  }
   ...
   */
  public boolean is_key_in_tree(int key) {
	  IAVLNode r = this.getRoot();
	  while (true) {
		  if (!(r.isRealNode())) {
			  return false;
  ...
  private int[] sortedKeyArray(IAVLNode r) {
	  ...
	  int new_array_size = 1 + left_subtree_keys.length + right_subtree_keys.length; //Replace length with size!!!
  ...
  private String[] sortInfoRec(IAVLNode v) {
   ...
   public int join(IAVLNode x, AVLTree t)
   {
	   ...
	   if(t_height >= h) { // if rank(t) >= this rank
		   ...
		   this.rebalance(x);
		   t.maxKeyNode = this.Select(root, size());
		   t.minKeyNode = this.Select(root, 0);
		   return t_height - h + 1 ;
	   }
	   // rank(t) <= this rank :
	   IAVLNode z = r;
